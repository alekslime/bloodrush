...
func _physics_process(delta: float) -> void:
	...
	if recoil.length() > 0:
		recoil = recoil.lerp(Vector2.ZERO, delta * recoil_recovery_speed)
		rot.x -= recoil.y * delta * 10  # Apply vertical recoil
		rot.y -= recoil.x * delta * 10  # Apply horizontal recoil
		
		# Clamp vertical rotation again after applying recoil
		rot.x = clamp(rot.x, -y_limit, y_limit)
...
# Apply recoil from gun
func apply_recoil(amount: float) -> void:
	# Apply random recoil
	var recoil_x = randf_range(-amount * 0.5, amount * 0.5)
	var recoil_y = randf_range(-amount, -amount * 0.5)  # Mostly upward recoil
	recoil += Vector2(recoil_x, recoil_y)
