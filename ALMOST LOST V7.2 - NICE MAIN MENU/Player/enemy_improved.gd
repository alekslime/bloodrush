...
func _ready():
	...
	add_to_group("enemies")  # Important: Add to enemies group for wave tracking
...
func die():
	...
	# IMPORTANT: Remove from enemies group when dead
...
# Add this to enemy_improved.gd
func set_difficulty_multiplier(multiplier: float):
	max_health *= multiplier
	current_health = max_health
	damage *= multiplier
	move_speed *= (1.0 + (multiplier - 1.0) * 0.5)  # Scale speed more conservatively
	
	print("Enemy difficulty set to ", multiplier, " - Health: ", max_health, " Damage: ", damage)
